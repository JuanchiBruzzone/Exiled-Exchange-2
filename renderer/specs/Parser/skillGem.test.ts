...
import { testExports, parseClipboard } from "@/parser/Parser";
import {
  MetaSkillGem,
  RareItem,
  UncutSkillGem,
...
describe("isItemMissingItemClass", () => {
  ...
  it("so should now be false for meta skill gem", () => {
    const sections = testExports.itemTextToSections(MetaSkillGem.rawText);
    expect(testExports.isItemMissingItemClass(sections[0])).toBeTruthy();
  });

  it("should return false for any other item", () => {
    const sections = testExports.itemTextToSections(RareItem.rawText);
    expect(testExports.isItemMissingItemClass(sections[0])).toBe(false);
  });
});
