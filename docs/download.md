...
3. Copy `apt-data` from your old Awakened PoE Trade data directory into Exiled Exchange Native's data directory.
4. Edit `config.json` and set `"windowTitle": "Path of Exile 2"`.
5. Start Exiled Exchange 2 Native and PoE2.

Typical native data location on Linux is under `~/.config/exiled-exchange-native/apt-data`.
